        ...
        try {
          switch (event.type) {
            case "checkout.session.completed": {
              ...
              const orgId =
                s.metadata?.organization_id ?? (await orgIdFromCustomer(s.customer as string));
            ...
            case "customer.subscription.deleted": {
              ...
              const orgId =
                sub.metadata?.organization_id ?? (await orgIdFromCustomer(sub.customer as string));
              if (orgId) {
                const planId = await planIdFromPrice(sub.items.data[0]?.price.id);
          ...
          return new Response(err instanceof Error ? err.message : "Handler error", {
            status: 500,
          });
        }
      },
