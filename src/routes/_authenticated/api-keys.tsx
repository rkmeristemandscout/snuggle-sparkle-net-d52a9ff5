  ...
  const { data: keys = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("api_keys")
        .select(
          "id, name, prefix, scopes, last_used_at, usage_count, revoked_at, expires_at, created_at",
        )
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Manage programmatic access to this organization.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Give the key a memorable name. The secret is shown once.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              if (name.trim().length >= 2) createKey.mutate();
            }}
          >
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
                      size="sm"
                      disabled={!!k.revoked_at || regenKey.isPending}
                    ...
                    <Button
                      variant="destructive"
                      size="sm"
                      disabled={!!k.revoked_at || revokeKey.isPending}
