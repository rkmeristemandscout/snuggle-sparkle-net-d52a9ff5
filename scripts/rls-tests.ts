...
async function main() {
  ...
  // --- Create orgs ---
  const orgA = (
    await A.rpc("create_organization", { _name: `Org A ${tag}`, _slug: slugA, _description: "A" })
  ).data as any;
  const orgB = (
    await B.rpc("create_organization", { _name: `Org B ${tag}`, _slug: slugB, _description: "B" })
  ).data as any;
  ...
  try {
    // --- Slug uniqueness ---
    const dup = await A.rpc("create_organization", {
      _name: "dup",
      _slug: slugB,
      _description: "",
    });
    ...
    const aSeesBmembers = await A.from("organization_members")
      .select("id")
      .eq("organization_id", orgB.id);
    expect("A cannot SELECT B's members", (aSeesBmembers.data ?? []).length === 0);

    // --- Tenant-isolated UPDATE / DELETE ---
    const aUpdB = await A.from("organizations")
      .update({ name: "hacked" })
      .eq("id", orgB.id)
      .select();
    expect("A cannot UPDATE B's org", (aUpdB.data ?? []).length === 0);
    const bCheck = (await admin.from("organizations").select("name").eq("id", orgB.id).single())
      .data as any;
    ...
    const aInsBmember = await A.from("organization_members")
      .insert({ organization_id: orgB.id, user_id: idA, role: "admin" })
      .select();
    expect(
      "A cannot INSERT self into B's org",
      !!aInsBmember.error || (aInsBmember.data ?? []).length === 0,
    );
    ...
    const aOwnMem = await A.from("organization_members")
      .select("role")
    ...
    const invIns = await A.from("organization_invitations")
      .insert({
    ...
    const bSeesInv = await B.from("organization_invitations")
      .select("id")
      .eq("token", token1)
      .maybeSingle();
    ...
    // --- Reject flow ---
    const invIns2 = await A.from("organization_invitations")
      .insert({
        organization_id: orgA.id,
        email: emailB,
        role: "member",
    ...
    const bStillOutsider = await B.from("organizations")
      .select("id")
      .eq("id", orgA.id)
      .maybeSingle();
    ...
    const bAsMember = await B.from("organization_members")
      .select("role")
      .eq("organization_id", orgA.id)
      .eq("user_id", idB)
      .maybeSingle();
    ...
    const bUpd = await B.from("organizations")
      .update({ name: "member-hacked" })
      .eq("id", orgA.id)
      .select();
    expect("Member cannot UPDATE org", (bUpd.data ?? []).length === 0);
    const bPromote = await B.from("organization_members")
      .update({ role: "owner" })
      .eq("organization_id", orgA.id)
      .eq("user_id", idB)
      .select();
    ...
    const aPromoteB = await A.from("organization_members")
      .update({ role: "admin" })
      .eq("organization_id", orgA.id)
      .eq("user_id", idB)
      .select();
    expect(
      ...
      (aPromoteB.data ?? []).length === 1,
      aPromoteB.error?.message,
    );

    // Now B (admin) can UPDATE org
    const bUpd2 = await B.from("organizations")
    ...
    const aDemote = await A.from("organization_members")
      .update({ role: "member" })
      .eq("organization_id", orgA.id)
      .eq("user_id", idB)
      .select();
    expect("Owner can demote admin→member", (aDemote.data ?? []).length === 1);
    const bUpd3 = await B.from("organizations")
    ...
    expect(
      ...
      !!aLeave.error,
      aLeave.error?.message ?? "unexpectedly succeeded",
    );
    const aSelfDemote = await A.from("organization_members")
      .update({ role: "admin" })
      .eq("organization_id", orgA.id)
      .eq("user_id", idA)
      .select();
    expect(
      "Last owner cannot self-demote",
      !!aSelfDemote.error || (aSelfDemote.data ?? []).length === 0,
    );
